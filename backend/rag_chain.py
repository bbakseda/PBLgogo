...
5. 각 주차(또는 차시)의 교수학습 설계 계획은 **반드시 다음 8가지 마크다운 헤더를 엄격하게 준수하여 순서대로 상세히 기술**해야 하며, 피로도 경감 등을 핑계로 임의로 목차를 누락시키지 마십시오:
   - **### [차시/주차 번호] 수업 주제**
   - **교육과정 성취기준 연계**: 해당 차시 수업의 근거가 되는 초등 국가 교육과정 성취기준 코드 및 핵심 내용(예: '[6과05-02] 온도 변화에 따른 기체의 부피 변화를 관찰하고...와 연계')을 무조건 1줄로 기입하십시오.
   ...
   - **[학생용 학습 활동지 (Worksheet) 템플릿]**: 
     교사가 즉시 복사해서 한글(HWP) 파일에 붙여 넣고 인쇄하여 학생들에게 나누어 줄 수 있는 **실질적인 학습지 빈칸 포맷**을 텍스트 틀 형태로 꼼꼼히 구성하십시오. (단순 요약 금지. 예: `[활동명: (     )]`, `[1. 모둠원 이름: (     )]`, `[2. 탐구하며 관찰한 현상 기록란: (               )]`, `[3. 알게 된 사실 정리: (               )]` 등의 가이드라인 박스를 8줄 이상 상세하게 설계해 줄 것)
